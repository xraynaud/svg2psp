...
cut.psp = function(datapsp,maxlength) {
  ...
  for (i in 1:datapsp$n) {
    seg = datapsp[i]
    while (lengths.psp(seg) > maxlength) {
      pts[n,]$mark = marks(seg)
      pts[n,]$x0 = seg$ends$x0
      pts[n,]$y0 = seg$ends$y0
      x = (maxlength/lengths.psp(seg))*( seg$ends$x1 -  seg$ends$x0) +  seg$ends$x0
      y = (maxlength/lengths.psp(seg))*( seg$ends$y1 -  seg$ends$y0) +  seg$ends$y0
      pts[n,]$x1 = x
      pts[n,]$y1 = y
      seg$ends$x0 = x
      seg$ends$y0 = y
  ...
  newpsp = as.psp(pts[,1:4],check=F,window=as.owin(datapsp))
